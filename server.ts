...
// Robust fallback wrapper to route around 503 (model overloaded), 429 (rate limits/quota) and other transient errors
async function generateContentWithFallback(
  ...
  for (const modelName of uniqueModels) {
    try {
      ...
      console.log(`[Gemini Fallback System] Model ${modelName} encountered rate limit or quota limit: ${errMsg.substring(0, 150)}. Retrying with alternative model...`);
    }
  }
  throw lastError;
}
...
app.post("/api/analyze-script", async (req, res) => {
  ...
  if (isKeywordsOnly) {
    ...
    try {
      ...
      console.log("[Info] Reel Dreamer AI engaged local fallback generator:", e?.message || e);
...
"""`;
    ...
    console.log("[Info] Gemini script parser engaged local backup generator:", error?.message || error);
...
Respond with valid JSON structure matching the specified schema.`;
    ...
    console.log("[Info] Copilot AI engaged local fallback message:", error?.message || error);
    const isAmharic = /[\u1200-\u137F]/.test(message || "");
    return res.json({
      message: isAmharic 
        ? "ሰላም! ዮቶር የግል ኤአይ ረዳት ነኝ። የኤአይ ሞዴል አገልግሎት በጊዜያዊነት በኮታ ገደብ ላይ በመሆኑ አውቶማቲክ ረዳት ገፅታ ተነስቷል። እባክዎን በጥቂት ሰከንዶች ውስጥ እንደገና ይሞክሩ።"
        : "Hello! Yotor Personal AI assistant here. The AI model service is temporarily at its quota limit, so automatic offline mode was engaged. Please try again in a few moments.",
      updateConfig: {},
      updateScenes: { action: "none" }
    });
  }
...
app.post("/api/analyze-video", async (req, res) => {
  ...
  try {
    ...
    console.log(`[Video Analyzer] Requesting video understanding from Gemini...`);
    const result = await generateContentWithFallback(ai, {
      model: "gemini-2.5-flash",
      contents: [
        {
    ...
    console.log("[Video Analyzer] Handled exception:", error?.message || error);
