const fs = require('fs');
const content = fs.readFileSync('src/components/RenderModal.tsx', 'utf8');

const regex = /<div className="space-y-1">\s*<span className="text-zinc-600 uppercase tracking-widest text-\[8px\] font-mono block">\s*\{language === 'am' \? 'የፋይል መጠ\s*<div className="flex items-center gap-3">/g;
...
fs.writeFileSync('src/components/RenderModal.tsx', patched);
